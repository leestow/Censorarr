(() => {
  ...
  function addStyles(){
    ...
    style.textContent=`
      ...
      .fs-wiki-group{margin:0 0 15px}
      .fs-wiki-group:last-child{margin-bottom:0}
      ...
      .settings-page[data-settings="general"] .fs-ai-dialogue-rules{
        max-width:none!important;margin-top:14px!important;padding:17px 18px!important;border-radius:10px!important;
      ...
      .settings-page[data-settings="general"] .fs-ai-rule{
        min-width:0;padding:2px 16px 5px!important;background:transparent!important;border:0!important;border-radius:0!important;
      }
      ...
      .settings-page[data-settings="general"] .fs-ai-dialogue-grid+.fs-ai-dialogue-grid:before{
        content:'Behavior & fallback';grid-column:1/-1;color:var(--muted);font-size:10px;font-weight:900;letter-spacing:.08em;text-transform:uppercase;margin:0 0 10px;
      }
      ...
      @media(max-width:1050px){
        ...
        .settings-page[data-settings="general"] .fs-source-rules-grid,
        .settings-page[data-settings="general"] .fs-ai-dialogue-grid{grid-template-columns:1fr!important;gap:0!important}
        .settings-page[data-settings="general"] .fs-source-rule,
        .settings-page[data-settings="general"] .fs-ai-rule{padding:12px 0!important;border-left:0!important;border-top:1px solid var(--line)!important}
        .settings-page[data-settings="general"] .fs-source-rule:first-child,
        .settings-page[data-settings="general"] .fs-ai-rule:first-child{border-top:0!important;padding-top:2px!important}
      }
  ...
  function organizeWiki(){
    ...
    const buttons=qa('.fs-wiki-item[data-wiki-slug]',box);
    ...
    for(const b of buttons){
      ...
      if(/^_/.test(slug) || /^_?sidebar$/i.test(slug) || /^_?footer$/i.test(slug)){
        b.classList.add('fs-wiki-plumbing');
        continue;
      }
      usable.push(b);
    }
    ...
    for(const [title,test] of GROUPS){
      const items=usable.filter(b=>!assigned.has(b) && test(String(b.dataset.wikiSlug||'')));
      ...
      const list=document.createElement('div');list.className='fs-wiki-group-items';
      for(const b of items){assigned.add(b);list.appendChild(b)}
    ...
    if(other.length){
      ...
      const list=document.createElement('div');list.className='fs-wiki-group-items';other.forEach(b=>list.appendChild(b));
      group.append(head,list);frag.appendChild(group);
    }
    box.innerHTML='';box.appendChild(frag);box.dataset.fsGrouped='1';
  }

  function markWikiDirty(){
    const box=q('#fsWikiList');
    if(box) box.dataset.fsGrouped='0';
    organizeWiki();
  }

  function polishGeneral(){
    const page=q('.settings-page[data-settings="general"]');
    if(!page) return;
    page.classList.add('fs-general-polished');
    const source=q('.fs-source-rules',page);if(source) source.dataset.polished='1';
    const ai=q('.fs-ai-dialogue-rules',page);if(ai) ai.dataset.polished='1';
  }

  function apply(){addStyles();markWikiDirty();polishGeneral()}

  const observer=new MutationObserver(muts=>{
    let wikiChanged=false,generalChanged=false;
    for(const m of muts){
      const t=m.target instanceof Element?m.target:m.target?.parentElement;
      if(t?.id==='fsWikiList' || t?.closest?.('#fsWikiList')) wikiChanged=true;
      if(t?.closest?.('.settings-page[data-settings="general"]') || qa('.settings-page[data-settings="general"]',t||document).length) generalChanged=true;
    }
    if(wikiChanged){const box=q('#fsWikiList');if(box)box.dataset.fsGrouped='0';queueMicrotask(organizeWiki)}
    if(generalChanged) queueMicrotask(polishGeneral);
  });
